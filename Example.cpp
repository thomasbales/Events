...
int main()
{
    ...
    //callback function signatures the event will be calling in angle brackets.
    //Events are only compatible with callbacks with a return type of void.

    Event<> event;
    Event<int, string> eventWithParams;

    //Add callbacks with either the Event.AddCallback function or the += operator.
    ...
    //Remove callbacks with either the Event.RemoveCallback function or the -= operator.
